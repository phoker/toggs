...
const Knob = styled(animated.div)`
  border: ${({ theme: { knobColor, knobRadius }, width }) =>
    `${knobColor} ${knobRadius * width}px solid`};
  ...
  transition: transform ${defaultDuration}s cubic-bezier(0.4, 0, 0.2, 1),
  ...
  &:active {
    &:before {
      transform: ${({ theme: { knobHoverScale } }) =>
        `scale(${knobHoverScale})`};
    }
  }
  &:before {
    ...
    top: ${({
      theme: {
        knobShadowPosition: { top }
      },
      width
    }) => `${top * width}px`};
    left: ${({
      theme: {
        knobShadowPosition: { left }
      },
      width
    }) => `${left * width}px`};
    width: ${({ theme: { knobShadowWidth }, width }) =>
      `${knobShadowWidth * width}px`};
    height: ${({ theme: { knobShadowHeight }, width }) =>
      `${knobShadowHeight * width}px`};
    transform: scale(0);
    transition: transform ${defaultDuration}s cubic-bezier(0.4, 0, 0.2, 1),
      background-color ${defaultDuration}s cubic-bezier(0.4, 0, 0.2, 1);
    border-radius: 50%;
    opacity: ${({ checked }) => (checked ? '.2' : '.15')};
    content: '';
  }
`
